import { ChevronLeft, MapPin, Navigation, Building2, Home } from "lucide-react";

interface PickupEditorScreenProps {
  onBack: () => void;
  onSelectLocation: (location: string) => void;
  currentLocation: string;
}

const PickupEditorScreen = ({ onBack, onSelectLocation, currentLocation }: PickupEditorScreenProps) => {
  const savedPlaces = [
    { 
      id: "current", 
      label: "Current Location", 
      address: currentLocation,
      icon: Navigation,
      color: "text-primary",
      bgColor: "bg-primary/20"
    },
    { 
      id: "home", 
      label: "Home", 
      address: "Al Malqa District",
      icon: Home,
      color: "text-success",
      bgColor: "bg-success/20"
    },
    { 
      id: "work", 
      label: "Work", 
      address: "King Saud University",
      icon: Building2,
      color: "text-info",
      bgColor: "bg-info/20"
    },
  ];

  const nearbyPlaces = [
    { id: "gate1", label: "Main Gate", address: "KSU Gate 1" },
    { id: "gate2", label: "East Gate", address: "KSU Gate 2" },
    { id: "library", label: "Library", address: "Central Library" },
  ];

  return (
    <div className="flex flex-col items-center h-full w-full animate-scale-in">
      {/* Header */}
      <div className="flex items-center w-full px-2 pt-2 pb-2">
        <button 
          onClick={onBack}
          className="w-8 h-8 rounded-full bg-secondary/50 flex items-center justify-center"
          aria-label="Go back"
        >
          <ChevronLeft className="w-5 h-5 text-foreground" />
        </button>
        <span className="text-sm font-semibold text-foreground flex-1 text-center pr-8">Pickup</span>
      </div>

      {/* Saved places */}
      <div className="flex flex-col gap-1.5 w-full px-3 flex-1 overflow-y-auto">
        <p className="text-[9px] text-muted-foreground uppercase tracking-wider mb-1">Saved Places</p>
        {savedPlaces.map((place, index) => (
          <button
            key={place.id}
            onClick={() => onSelectLocation(place.address)}
            className="flex items-center gap-2 w-full p-2 rounded-xl bg-secondary/40 hover:bg-secondary/60 transition-all animate-fade-in-up"
            style={{ animationDelay: `${index * 0.05}s` }}
            aria-label={`Select ${place.label} as pickup`}
          >
            <div className={`w-8 h-8 rounded-full ${place.bgColor} flex items-center justify-center shrink-0`}>
              <place.icon className={`w-4 h-4 ${place.color}`} />
            </div>
            <div className="flex flex-col items-start flex-1 min-w-0">
              <span className="text-[11px] font-medium text-foreground">{place.label}</span>
              <span className="text-[9px] text-muted-foreground truncate w-full text-left">{place.address}</span>
            </div>
          </button>
        ))}

        {/* Nearby places */}
        <p className="text-[9px] text-muted-foreground uppercase tracking-wider mt-2 mb-1">Nearby</p>
        {nearbyPlaces.map((place, index) => (
          <button
            key={place.id}
            onClick={() => onSelectLocation(place.address)}
            className="flex items-center gap-2 w-full p-2 rounded-xl bg-secondary/30 hover:bg-secondary/50 transition-all animate-fade-in-up"
            style={{ animationDelay: `${(index + 3) * 0.05}s` }}
            aria-label={`Select ${place.label} as pickup`}
          >
            <div className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center shrink-0">
              <MapPin className="w-4 h-4 text-muted-foreground" />
            </div>
            <div className="flex flex-col items-start flex-1 min-w-0">
              <span className="text-[11px] font-medium text-foreground">{place.label}</span>
              <span className="text-[9px] text-muted-foreground truncate w-full text-left">{place.address}</span>
            </div>
          </button>
        ))}
      </div>
    </div>
  );
};

export default PickupEditorScreen;

